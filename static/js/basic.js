    ...
    $.fn.extend({
        showFormError: function (err) {
            return this.each(function () {
                ...
                if (err) {
                    $alert.text((err.message ? err.message : (err.error ? err.error /*uikit显示错误*/: err))).removeClass('uk-hidden').show();  //可以显示Appointed2发送的错误
...
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) {
        ...
        //可以检查是Appointed2发送的错误
        var org_message = ''
        if(jqXHR.responseJSON && jqXHR.responseJSON.data.message)
        {
            org_message = jqXHR.responseJSON.data.message;
        }

        return callback({'error': 'http_bad_response', 'data': '' + jqXHR.status, 'message': '出现了问题 (HTTP ' + jqXHR.status + ')' + '，服务器的消息:' + org_message + '。'});
    });
}
